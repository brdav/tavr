...
class SyntheticDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.cat_table_prior = np.array(
            [0.18, 0.15, 0.21, 0.73, 0.25, 0.56, 0.29, 0.52, 0.81, 0.93, 0.21, 0.20, 0.03, 0.31, 0.04])
        ...
        self.beta = 0.1 * \
            rs.randn(self.nr_table_features, self.nr_image_features) + 0.1
        ...
        self.log_var_y = 0.0  # scalar, let's keep this one fixed
        ...
        self.gt_model = {
            ...
            'phi': self.phi,
            'b_Y': self.b_Y,
            'sigma_y': np.sqrt(np.exp(self.log_var_y)),
            'sigma_I': np.sqrt(np.exp(self.log_var_I)),
            'sigma_J': np.sqrt(np.exp(self.log_var_J))
        }
        ...
        if self.missing_mode == 'debug':
            ...
            self.aux_missing_p = [0, 0, 0, 0, 0, 0,
            ...
            self.cont_missing_p = [0.002, 0.003, 0.084,
                                   0.080, 0.014, 0.132, 0.128, 0.159, 0.166, 0.195]
            self.cat_missing_p = [0.004, 0.010, 0.015, 0.061, 0.264, 0.018,
                                  0.009, 0.017, 0.317, 0.023, 0.005, 0.006, 0.087, 0.001, 0.047]
            self.aux_missing_p = [0.072, 0.072, 0.072, 0.228, 0.070, 0.063,
                                  0.118, 0.074, 0.073, 0.068, 0.071, 0.487, 0.413, 0.509, 0.300]
            self.image_missing_p = 0.49
            if missing_factor is not None:
                self.cont_missing_p = [min(1.0, el * missing_factor)
                                       for el in self.cont_missing_p]
                self.cat_missing_p = [min(1.0, el * missing_factor)
                                      for el in self.cat_missing_p]
                self.aux_missing_p = [min(1.0, el * missing_factor)
                                      for el in self.aux_missing_p]
                self.image_missing_p = min(
                    1.0, self.image_missing_p * missing_factor)
            ...
            self.cont_missing_masks = np.load(os.path.join(
                dirname, 'tavi_statistics', 'cont_missing_masks.npy'))
            self.cat_missing_masks = np.load(os.path.join(
                dirname, 'tavi_statistics', 'cat_missing_masks.npy'))
            self.aux_missing_masks = np.load(os.path.join(
                dirname, 'tavi_statistics', 'aux_missing_masks.npy'))
            self.image_missing_masks = np.load(os.path.join(
                dirname, 'tavi_statistics', 'image_missing_masks.npy'))
    ...
    def _sample_datapoint(self):
        ...
        cat = np.random.binomial(
            n=1, p=self.cat_table_prior, size=self.nr_cat_features).astype(np.float32) * 2 - 1.
        A = np.concatenate((cont, cat), axis=0)
        f_img = np.sqrt(np.exp(self.log_var_I)) * \
            np.random.randn(self.nr_image_features) + \
            np.transpose(self.beta) @ A
        J = np.sqrt(np.exp(self.log_var_J)) * \
            np.random.randn(self.nr_aux) + np.transpose(self.phi) @ f_img
        logit_y = np.transpose(self.alpha_I) @ (f_img - np.transpose(self.beta)
                                                @ A) + np.transpose(self.alpha_A) @ A + self.b_Y
        y = self._boltzmann_sample(logit_y, np.exp(self.log_var_y))
        ...
        if self.missing_mode == 'debug':
            ...
            if mode in [1, 4, 5, 7]:
                ...
                num_missing = np.random.randint(
                    1, min(8, self.nr_cont_features + 1))
            ...
            if mode in [2, 4, 6, 7]:
                # have at most 8 missing entries
                num_missing = np.random.randint(
                    1, min(8, self.nr_cat_features + 1))
            ...
            cont_missing_mask = np.random.rand(
                self.nr_cont_features) < self.cont_missing_p
            cat_missing_mask = np.random.rand(
                self.nr_cat_features) < self.cat_missing_p
            aux_missing_mask = np.random.rand(self.nr_aux) < self.aux_missing_p
            ...
            mode = self._get_mode(
                has_missing_cont, has_missing_cat, image_missing)
        elif self.missing_mode == 'emulate_tavi':
            ...
            mode = self._get_mode(
                has_missing_cont, has_missing_cat, image_missing)
        return cont_missing_mask, cat_missing_mask, aux_missing_mask, image_missing, mode
